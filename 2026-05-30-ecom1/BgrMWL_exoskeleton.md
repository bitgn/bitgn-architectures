---
source_code: https://github.com/muxx/bitgn-ecom1-exoskeleton
run_ids:
  - run-22RzsrSdFkaiCzLAHF9MrN8om
  - run-22RyGBseMXrbMuePPL6nipFfP
  - run-22Rxz4Tk23dZCqBSRLf6zsuva
model_names:
  - gpt-5.4-mini
  - gpt-5.4-nano
author: Ilyas Salikhov
author_linkedin: https://www.linkedin.com/in/salikhovilyas/
author_github: https://github.com/muxx
author_telegram: dev_salikhov
impact: Wraps a small GPT-5.4 mini dispatcher in deterministic preflight checks, domain helpers, an evidence ledger, reference normalization, and a final formatter to make ECOM1 decisions fast, grounded, and policy-safe.
challenge: ecom
---
...
![Exoskeleton architecture: four stages. Purple marks nodes that run on the model, neutral marks deterministic code, blue marks environment tools and helpers](res/BgrMWL-exoskeleton-01-architecture.png)
...
![A single step of the main loop: the dispatcher calls tools and accumulates context until it calls report_completion](res/BgrMWL-exoskeleton-02-step-loop.png)
...
![Run heatmap](res/BgrMWL-score-dashboard.png)
...
![Security gate: the model extracts the intent, the code makes the decision — by "blast radius" and /bin/id](res/BgrMWL-exoskeleton-03-security-gate.png)
...
![The grounding-reference assembly pipeline: canonicalization, auto-add, filtering, shielding the private](res/BgrMWL-exoskeleton-04-grounding-refs.png)
...
![A two-speed improvement loop: first the architecture by hand, then autonomous refinement of components](res/BgrMWL-exoskeleton-05-improvement-loop.png)
